...
class Accounts(commands.Cog):
    ...
    async def account_(self, ctx, user: discord.Member = None):
        ...
        try:
            ...
            if account.pets:
                embed.add_field(name="Currently earning", value=f"${sum((p.earns*p.level*self.bot.pet_species[p.type][p.species][1]/pet.level*2) for p in account.pets)}/30 minutes")
