...
class EmbedPaginator:
    ...
    async def paginate(self):
        ...
        while self.looping:
            try:
                try:
                    reaction, _ = await self.ctx.bot.wait_for("reaction_add", timeout=600.0,
                                                              check=lambda r, u: u == self.ctx.author and str(
                                                                  r.emoji) in ["\u2b05", "\u27a1", "\u23f9"] and r.message.id == self.message.id)
